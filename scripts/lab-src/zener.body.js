...
function makeZener3D(){
  const g=new THREE.Group();
  const body=new THREE.Mesh(new THREE.CylinderGeometry(0.045,0.045,0.16,16),MAT.zenerBody);
  body.rotation.z=Math.PI/2;g.add(body);
  const band=new THREE.Mesh(new THREE.CylinderGeometry(0.047,0.047,0.02,16),MAT.zenerBand);
...
zenG.position.set(0.15,0.24,1.15);zenG.rotation.y=Math.PI/2;
zenG.userData.act='zen3d';zenG.userData.title='Diodo zener';
benchG.add(zenG);
const rlG=makeResistor3D();
rlG.position.set(0.45,0.24,1.15);
...
function refreshAll(){
  ...
  const zone=r.v<=V0v?'cutoff':(r.v<=VZKv?'knee':(r.iz>z.izm?'over':'valid'));
  const vm=vAtIzm(z);
