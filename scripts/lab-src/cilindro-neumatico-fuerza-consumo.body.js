...
const MATP={aluminio:MAT.plate, acero:MAT.barrel, cobre:MAT.plate, cromo:MAT.rod,
  chapa:MAT.plate, hierro:MAT.body, negro:MAT.valve, goma:MAT.valve,
  blanco:MAT.rod, ceramica:MAT.rod, rojo:std(0xc0392b,0.5,0.3)};
...
/* EL CILINDRO, con lo que tiene un cilindro ISO 15552 de verdad: los CUATRO
   TIRANTES que sujetan las tapas —la presión empuja las dos tapas hacia fuera y
   son ellos los que aguantan, por eso este cilindro se puede desarmar—, el
   cabezal TALADRADO por donde sale el vástago, y una toma a cada lado del
   émbolo, que es lo que hace visible que las dos áreas son distintas y que de
   ahí sale toda la práctica. Se REHACE al cambiar de catálogo: el diámetro no
   se estira, porque estirar la malla estiraría también los tirantes. */
const barrel=new THREE.Group();
barrel.rotation.z=-Math.PI/2;              // el eje del cilindro, en +x
barrel.position.set(2.30,0.86,0.95);scene.add(barrel);
...
let cilP3=null, cilKey='';
const CARRERA3D=0.42;
...
/* LA VÁLVULA 5/2, con sus dos solenoides y su base de placa. El pulsador rojo
   de la punta de cada bobina no es adorno: es el accionamiento manual de
   emergencia, y es lo primero que se toca para saber si una máquina parada lo
   está por la parte eléctrica o por la neumática. */
const valv=P3.valvulaDireccional(MATP,{ancho:0.52,alto:0.15,fondo:0.26,vias:5});
valv.position.set(2.30,0.66,0.20);
valv.traverse(o=>{if(o.isMesh)o.castShadow=true;});scene.add(valv);
...
/* EL REGULADOR, con su sombrerete de membrana y su mando moleteado. Girar el
   mando no fija ninguna presión: aprieta un muelle, y el equilibrio entre ese
   muelle y la presión de salida es lo que da el valor. */
const reg=P3.reguladorPresion(MATP,{d:0.22,alto:0.44});
reg.position.set(1.35,0.52,0.30);
reg.traverse(o=>{if(o.isMesh)o.castShadow=true;});scene.add(reg);
...
function updateHW(){
  ...
  if(cilKey!==cfg.cil){
    cilKey=cfg.cil;
    if(cilP3){ barrel.remove(cilP3); cilP3.traverse(o=>{ if(o.isMesh) o.geometry.dispose(); }); }
    cilP3=P3.cilindroHidraulico(MATP,
      {d:2*r,carrera:CARRERA3D,vastago:2*rr,tirantes:4,horquilla:false});
    cilP3.traverse(o=>{if(o.isMesh)o.castShadow=true;});
    barrel.add(cilP3);
  }
  const p=animP;
  // El vástago sale por el +y LOCAL de la pieza, que es el +x del banco.
  cilP3.userData.vastago.position.y=0.30*p;
  const dC=2*r;
  const xPunta=2.30+(CARRERA3D+0.85*dC)/2+(CARRERA3D+0.62*dC)-0.30*dC+0.30*p;
  tip.position.x=xPunta;
  carga.position.x=xPunta+0.26;
...
S.setAnimate((dt)=>{
  ...
  animP=ph<1?ph:2-ph;
  const p=animP;
  // El vástago va dentro de la pieza: se mueve su grupo, no una malla suelta.
  if(cilP3){
    cilP3.userData.vastago.position.y=0.30*p;
    const dC=cilP3.userData.d;
    const xPunta=2.30+(CARRERA3D+0.85*dC)/2+(CARRERA3D+0.62*dC)-0.30*dC+0.30*p;
    tip.position.x=xPunta; carga.position.x=xPunta+0.26;
  }
});
