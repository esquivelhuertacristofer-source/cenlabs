...
function drawFuenteScreen(){
  ...
  cx.fillText('Vs_rms = '+(RES?RES.vsecRms.toFixed(2):'—')+' V',cv.width/2,156);
...
function refreshQuestion(){
  ...
  el('dxbtns').innerHTML=q.opciones.map((o,i)=>`<button class="b" data-i="${i}">${'ABC'[i]}) ${o.t}</button>`).join('');
  Array.from(el('dxbtns').children).forEach(btn=>{
    btn.onclick=()=>{
      if(btn.classList.contains('right')||btn.classList.contains('wrong')) return;
      const o=q.opciones[+btn.dataset.i];
