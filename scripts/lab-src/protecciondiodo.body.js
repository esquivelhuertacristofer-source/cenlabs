...
function makeDiode3D(colorHex){
  const g=new THREE.Group();
  const body=new THREE.Mesh(new THREE.CylinderGeometry(0.045,0.045,0.16,16),std({color:colorHex,roughness:0.25,metalness:0.15}));
  body.rotation.z=Math.PI/2;g.add(body);
  const band=new THREE.Mesh(new THREE.CylinderGeometry(0.047,0.047,0.02,16),MAT.diodeBand);
...
function makeLed3D(){
  const g=new THREE.Group();
  const lens=new THREE.Mesh(new THREE.SphereGeometry(0.06,16,12),MAT.ledLens);
  g.add(lens);
  const halo=new THREE.Mesh(new THREE.SphereGeometry(0.1,12,10),MAT.ledHalo);
  g.add(halo);
  leads(g);
  g.userData.lens=lens;g.userData.halo=halo;
  return g;
}
const rsG=makeResistor3D();
rsG.position.set(-0.15,0.24,1.15);
rsG.userData.act='rs3d';rsG.userData.title='Resistor Rs';
benchG.add(rsG);
const ledG=makeLed3D();
ledG.position.set(0.18,0.26,1.15);
ledG.userData.act='dut3d';ledG.userData.title='Dispositivo bajo prueba';
benchG.add(ledG);
const diodeG=makeDiode3D(0x8a8a8a);
diodeG.position.set(0.18,0.24,1.15);diodeG.rotation.y=Math.PI/2;
