...
// --- resistencia de medida ---
const rBlock=roundedBox(0.24,0.14,0.16,MAT.plate,0.02);rBlock.position.set(2.95,0.58,0.62);scene.add(rBlock);
const rBand=new THREE.Mesh(new THREE.CylinderGeometry(0.045,0.045,0.20,14),MAT.copper);
rBand.rotation.z=Math.PI/2;rBand.position.set(2.95,0.58,0.62);scene.add(rBand);
...
const motor=new THREE.Mesh(new THREE.CylinderGeometry(0.16,0.16,0.42,20),MAT.steel);
motor.rotation.z=Math.PI/2;motor.position.set(0,0.22,0);pumpG.add(motor);
const volute=new THREE.Mesh(new THREE.CylinderGeometry(0.19,0.19,0.14,20),MAT.dark);
volute.rotation.x=Math.PI/2;volute.position.set(0.32,0.22,0);pumpG.add(volute);
const pumpBase=roundedBox(0.80,0.06,0.34,MAT.plate,0.02);pumpBase.position.set(0.10,0.03,0);pumpG.add(pumpBase);
motor.userData={title:'Grupo de bombeo',info:'El contactor lo arranca y lo para. Cada arranque calienta el motor: por eso el número de arranques por hora es un límite de diseño, no un capricho.'};
