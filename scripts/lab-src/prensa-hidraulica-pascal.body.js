...
const MATP={aluminio:MAT.steelDk, acero:MAT.steel, cobre:MAT.brass, cromo:MAT.chrome,
  chapa:MAT.steelDk, hierro:MAT.frame, negro:MAT.dark, goma:MAT.hose,
  blanco:MAT.chrome, ceramica:MAT.steelDk};
...
const CARRERA3D=0.22;

/* EL CILINDRO, con lo que tiene uno de verdad: los TIRANTES que sujetan las dos
   tapas —la presión empuja cada tapa hacia fuera y son ellos los que aguantan—,
   la tapa del vástago TALADRADA con su retén, y una toma a cada lado del
   émbolo. La toma de arriba y la de abajo no dan la misma fuerza aunque la
   presión sea la misma, y ver las dos es lo que hace evidente que el área es lo
   que manda. */
function buildCilindro(){
  const g=new THREE.Group();
  const inv=new THREE.Group(); inv.rotation.z=Math.PI; g.add(inv);   // el vástago, hacia abajo
  const cil=P3.cilindroHidraulico(MATP,{d:0.34,carrera:0.24,vastago:0.15,
    tirantes:4,horquilla:false,tomas:true});
  cil.traverse(o=>{ if(o.isMesh) o.castShadow=true; });
  inv.add(cil);
  const ram=cil.userData.vastago; ram.name='ram';
  const zap=new THREE.Mesh(new THREE.CylinderGeometry(0.115,0.115,0.06,24),MAT.steelDk);
  zap.position.y=0.611; zap.castShadow=true; ram.add(zap);           // la zapata que empuja
  const lb=labelSprite('Cilindro de trabajo','#8A94A0'); lb.position.set(0,0.52,0); lb.scale.multiplyScalar(0.56); g.add(lb);
  return g;
}
/* LA BOMBA, ABIERTA POR LA MITAD. Un bloque con una palanca encima no explica
   nada; lo que hay dentro sí: un émbolo pequeño y DOS VÁLVULAS ANTIRRETORNO de
   bola. Al subir el émbolo, la bola de aspiración se levanta y el cilindro se
   llena de aceite del depósito mientras la de impulsión se sienta y cierra el
   circuito; al bajar, se cambian los papeles y el aceite sale a presión. Ese
   par de bolas es todo el secreto de que una bomba de mano dé 700 bar: la
   presión se ACUMULA porque la de impulsión no la deja volver. Sin dibujarlas,
   la relación p = F_mano·R/A₁ parece que sale de la palanca sola.
   El émbolo se mueve con la palanca. */
function buildBomba(){
  const g=new THREE.Group();
  /* El cuerpo es una PLACA CON AGUJEROS: el taladro del émbolo y la cámara de
     la válvula de impulsión son huecos de verdad en la pieza, no dibujos
     encima. Por eso lo de dentro se ve DENTRO, metido en su alojamiento. */
  const cont=P3.contornoRedondeado([[-0.19,-0.08],[0.19,-0.08],[0.19,0.08],[-0.19,0.08]],0.028,3);
  const hBore=P3.contornoRedondeado([[-0.110,-0.062],[-0.034,-0.062],[-0.034,0.056],[-0.110,0.056]],0.012,3);
  const hImp =P3.contornoRedondeado([[-0.176,-0.046],[-0.140,-0.046],[-0.140,0.028],[-0.176,0.028]],0.010,3);
  const body=new THREE.Mesh(P3.extruido(cont,{huecos:[hBore,hImp],espesor:0.11,bisel:0.006}),MAT.steel);
  body.castShadow=true; g.add(body);
  // Galerías taladradas, marcadas en la cara del corte.
  const gal=(x,y,l,gira)=>{ const m=new THREE.Mesh(new THREE.CylinderGeometry(0.010,0.010,l,10),MAT.dark);
    m.position.set(x,y,0.050); m.rotation.z=gira; g.add(m); };
  gal(-0.072,-0.072,0.028,0);          // aspiración, desde el fondo
  gal(-0.125,-0.052,0.042,Math.PI/2);  // del taladro a la cámara de impulsión
  gal(-0.158,0.048,0.044,0);           // de la cámara al racor de salida
  gal(-0.176,0.062,0.038,Math.PI/2);
  /* LAS DOS VÁLVULAS ANTIRRETORNO de bola. Al subir el émbolo, la de aspiración
     se levanta y el taladro se llena de aceite del depósito mientras la de
     impulsión se sienta y cierra; al bajar se cambian los papeles y el aceite
     sale a presión. Ese par de bolas es todo el secreto de que una bomba de
     mano llegue a 700 bar: la presión se ACUMULA porque la de impulsión no la
     deja volver. Sin ellas, p = F_mano*R/A1 parece que sale de la palanca sola. */
  const valvula=(x,y)=>{
    const a=new THREE.Mesh(P3.revolucion([[0.009,0],[0.024,0.018],[0.024,0.026],[0.009,0.026]],
      {seg:18}),MAT.steelDk);
    a.position.set(x,y,0); g.add(a);
    const b=new THREE.Mesh(new THREE.SphereGeometry(0.0135,16,12),MAT.chrome);
    b.position.set(x,y+0.020,0); g.add(b);
    const mu=P3.muelle(MATP,{r:0.010,largo:0.024,vueltas:3,hilo:0.0032});
    mu.position.set(x,y+0.046,0); g.add(mu);
  };
  valvula(-0.072,-0.058);            // aspiración: abre hacia arriba
  valvula(-0.158,-0.038);            // impulsión: abre hacia el racor
  // El émbolo y su vástago, que es lo que la palanca mueve.
  const emb=new THREE.Group(); emb.name='embolo'; g.add(emb);
  const pis=new THREE.Mesh(new THREE.CylinderGeometry(0.034,0.034,0.046,20),MAT.chrome);
  pis.position.set(-0.072,0.014,0); pis.castShadow=true; emb.add(pis);
  for(let i=0;i<2;i++){ const jt=new THREE.Mesh(new THREE.TorusGeometry(0.034,0.0045,8,20),MAT.dark);
    jt.rotation.x=Math.PI/2; jt.position.set(-0.072,0.002+i*0.024,0); emb.add(jt); }
  const vst=new THREE.Mesh(new THREE.CylinderGeometry(0.0125,0.0125,0.170,14),MAT.chrome);
  vst.position.set(-0.072,0.122,0); emb.add(vst);
  const oj=new THREE.Mesh(new THREE.TorusGeometry(0.019,0.008,8,16),MAT.steelDk);
  oj.position.set(-0.072,0.203,0); emb.add(oj);
  const piv=new THREE.Group(); piv.name='lever'; piv.position.set(-0.145,0.215,0);
  const lever=new THREE.Mesh(new THREE.BoxGeometry(0.72,0.035,0.05),MAT.steelDk); lever.position.set(0.34,0.02,0); lever.castShadow=true; piv.add(lever);
  const grip=new THREE.Mesh(new THREE.CylinderGeometry(0.028,0.028,0.15,12),MAT.rubber); grip.rotation.z=Math.PI/2; grip.position.set(0.63,0.02,0); piv.add(grip);
  const bul=new THREE.Mesh(new THREE.CylinderGeometry(0.011,0.011,0.075,12),MAT.brass);
  bul.rotation.x=Math.PI/2; bul.position.set(0.073,-0.012,0); piv.add(bul);   // el bulon del embolo
  piv.rotation.z=-0.20; g.add(piv);
  const orej=new THREE.Mesh(new THREE.BoxGeometry(0.05,0.16,0.075),MAT.steel);
  orej.position.set(-0.145,0.150,0); g.add(orej);                              // la oreja del pivote
  const pin=new THREE.Mesh(new THREE.CylinderGeometry(0.020,0.020,0.11,12),MAT.brass); pin.rotation.x=Math.PI/2; pin.position.set(-0.145,0.215,0); g.add(pin);
  const out=new THREE.Mesh(new THREE.CylinderGeometry(0.026,0.026,0.13,12),MAT.brass); out.rotation.z=Math.PI/2; out.position.set(-0.245,0.062,0); g.add(out);
  const asp=new THREE.Mesh(new THREE.CylinderGeometry(0.024,0.024,0.09,12),MAT.brass); asp.position.set(-0.072,-0.115,0); g.add(asp);
  const lb=labelSprite('Bomba de palanca','#8A94A0'); lb.position.set(0,0.46,0); lb.scale.multiplyScalar(0.56); g.add(lb);
  return g;
}
/* EL DEPÓSITO, con el VISOR de nivel en un costado —que es por donde se mira si
   hay aceite, y la razón de que el aire entre cuando el nivel baja— y su tapón
   de llenado con respiradero: un depósito cerrado a cal y canto no dejaría
   salir el aceite, porque nada ocuparía su sitio. */
function buildDeposito(){
  const g=new THREE.Group();
  const dep=P3.deposito({...MATP,acero:MAT.tank},{ancho:0.46,alto:0.26,fondo:0.30,nivel:0.62});
  dep.position.y=-0.13; g.add(dep);
  g.userData.dep=dep;
  const lb=labelSprite('Depósito de aceite','#8A94A0'); lb.position.set(0,0.34,0); lb.scale.multiplyScalar(0.54); g.add(lb);
  return g;
}
/* LA LIMITADORA, también abierta. Dentro no hay electrónica: hay un CONO
   apretado contra su asiento por un MUELLE, y el mando sólo aprieta más o menos
   ese muelle. Por eso la limitadora no «sabe» la presión: la presión empuja el
   cono, y cuando gana al muelle, abre. Verlo es la diferencia entre entender el
   tarado y creer que es un número que se programa. La contratuerca está ahí por
   la misma razón que en la máquina: sin ella la vibración desajusta el tarado. */
function buildLimitadora(){
  const g=new THREE.Group();
  const cuerpo=new THREE.Mesh(P3.extruido(P3.contornoRedondeado(
    [[-0.085,-0.085],[0.085,-0.085],[0.085,0.085],[-0.085,0.085]],0.02,3),
    {espesor:0.075,bisel:0.005}),corte(MAT.brass));
  cuerpo.position.z=-0.0375; cuerpo.castShadow=true; g.add(cuerpo);
  const cav=new THREE.Mesh(P3.revolucion(
    [[0.028,-0.060],[0.034,-0.060],[0.034,0.080],[0.028,0.080]],
    {seg:20,fase:0.62,arco:Math.PI*2-1.24}),corte(MAT.steelDk));
  g.add(cav);
  const cono=new THREE.Mesh(P3.revolucion([[0,-0.030],[0.027,0.004],[0.027,0.026],[0,0.026]],{seg:20}),MAT.chrome);
  cono.position.y=-0.026; cono.castShadow=true; g.add(cono);
  const mu=P3.muelle(MATP,{r:0.024,largo:0.070,vueltas:5,hilo:0.006});
  mu.position.y=0.038; g.add(mu);
  const gal=new THREE.Mesh(new THREE.CylinderGeometry(0.012,0.012,0.100,10),MAT.dark);
  gal.rotation.z=Math.PI/2; gal.position.set(-0.045,-0.020,0); g.add(gal);
  const gal2=new THREE.Mesh(new THREE.CylinderGeometry(0.012,0.012,0.100,10),MAT.dark);
  gal2.rotation.z=Math.PI/2; gal2.position.set(0.045,0.010,0); g.add(gal2);
  const husillo=new THREE.Mesh(new THREE.CylinderGeometry(0.020,0.020,0.075,14),MAT.steelDk);
  husillo.position.y=0.105; g.add(husillo);
  const lock=new THREE.Mesh(new THREE.CylinderGeometry(0.038,0.038,0.016,6),MAT.steelDk); lock.position.y=0.098; g.add(lock);
  const knob=new THREE.Mesh(P3.revolucion([[0,0],[0.052,0.006],[0.044,0.062],[0.030,0.070],[0,0.070]],{seg:24}),MAT.red);
  knob.position.y=0.128; knob.castShadow=true; g.add(knob);
  for(let i=0;i<14;i++){ const a=i/14*Math.PI*2;    // moleteado del mando
    const c=new THREE.Mesh(new THREE.BoxGeometry(0.008,0.056,0.008),MAT.red);
    c.position.set(Math.cos(a)*0.048,0.160,Math.sin(a)*0.048); c.rotation.y=-a; g.add(c); }
  [-1,1].forEach(s=>{ const pt=new THREE.Mesh(new THREE.CylinderGeometry(0.024,0.024,0.10,12),MAT.brass); pt.rotation.z=Math.PI/2; pt.position.set(s*0.12,s<0?-0.020:0.010,0); g.add(pt); });
...
function buildManguera(){
  ...
  // Los TERMINALES prensados, con su casquillo hexagonal y su tuerca giratoria:
  // es la parte que no se puede fabricar en el taller y la que decide la
  // presión de trabajo de todo el latiguillo.
  [0,1].forEach(k=>{ const pt=pts[k?3:0], d=curve.getTangentAt(k?0.985:0.015).normalize();
    const term=new THREE.Group();
    const hex=new THREE.Mesh(new THREE.CylinderGeometry(0.044,0.044,0.055,6),MAT.brass); hex.position.y=0.030; term.add(hex);
    const casq=new THREE.Mesh(new THREE.CylinderGeometry(0.039,0.039,0.070,16),MAT.steelDk); casq.position.y=-0.030; term.add(casq);
    for(let i=0;i<5;i++){ const rz=new THREE.Mesh(new THREE.TorusGeometry(0.040,0.0045,6,18),MAT.steelDk);
      rz.rotation.x=Math.PI/2; rz.position.y=-0.056+i*0.013; term.add(rz); }
    term.position.copy(pt);
    term.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0), k?d:d.negate());
    term.traverse(o=>{ if(o.isMesh) o.castShadow=true; });
    g.add(term); });
  const lb=labelSprite('Latiguillo de alta presión','#8A94A0'); lb.position.set(-0.40,1.05,0); lb.scale.multiplyScalar(0.56); g.add(lb);
  return g;
}
/* EL MANÓMETRO de la casa: caja con bisel, esfera con divisiones —las largas
   cada cinco, como en uno de verdad, para poder leer entre marcas— y su racor
   hexagonal. La aguja conserva el nombre `needle` porque el lazo de animación
   la busca por él. */
function buildManometro(){
  const g=new THREE.Group();
  const man=P3.manometro(MATP,{d:0.25,divisiones:20,barrido:4.4,color:0xd64545});
  man.userData.aguja.name='needle';
...
function buildPrensa(){
  ...
  /* LOS AGUJEROS DE LOS MONTANTES y los PASADORES en los que se apoya la mesa.
     Es lo que distingue una prensa de taller de una mesa con un gato encima: la
     altura de trabajo no se ajusta girando nada, se elige cambiando los
     pasadores de agujero, y esos dos pasadores aguantan TODA la fuerza. */
  [-0.66,0.66].forEach(x=>{
    for(let i=0;i<9;i++){ const h=new THREE.Mesh(new THREE.CylinderGeometry(0.034,0.034,0.36,12),MAT.dark);
      h.rotation.x=Math.PI/2; h.position.set(x,0.42+i*0.155,0); g.add(h); }
    const pas=new THREE.Mesh(new THREE.CylinderGeometry(0.031,0.031,0.46,14),MAT.steelDk);
    pas.rotation.x=Math.PI/2; pas.position.set(x,0.699,0); pas.castShadow=true; g.add(pas);
    [-1,1].forEach(sz=>{ const cab=new THREE.Mesh(new THREE.CylinderGeometry(0.045,0.045,0.022,14),MAT.steelDk);
      cab.rotation.x=Math.PI/2; cab.position.set(x,0.699,sz*0.235); g.add(cab); });
  });
...
const PARTS=[
  {id:'cilindro',   name:'Cilindro de trabajo',    build:()=>buildCilindro(),   home:H(0,1.92,0),      note:'Es el que da la fuerza. Su área A₂ = πd²/4 convierte la presión del aceite en fuerza: F₂ = η·p·A₂. Cuanto mayor el diámetro, menos presión hace falta… y más aceite hay que meter.'},
  ...
  {id:'manometro',  name:'Manómetro',              build:()=>buildManometro(),  home:H(0.42,2.32,0.30),note:'Mide la presión del circuito, que es la MISMA en todo el fluido en reposo (ley de Pascal). Leerlo es la única forma de saber si la carga ya está exigiendo el máximo.'},
...
S.setAnimate((dt,t)=>{
  ...
  // El vástago sale por el +y LOCAL del cilindro, que está boca abajo: avanzar
  // es y creciente en su marco, aunque en el mundo baje.
  const cg=groups.cilindro; if(cg&&placed.cilindro){ const ram=cg.getObjectByName('ram'); if(ram) ram.position.y=ramNow*CARRERA3D; }
  // palanca de la bomba, y el émbolo que la palanca mueve: una bomba cuya
  // palanca sube y baja sola, sin que nada se mueva dentro, no bombea nada.
  if(pumping>0){ pumping=Math.max(0,pumping-dt); leverT+=dt*7; }
  const bg2=groups.bomba; if(bg2&&placed.bomba){
    const ang=-0.20+(pumping>0?0.30*Math.sin(leverT):0);
    const lv=bg2.getObjectByName('lever'); if(lv) lv.rotation.z=ang;
    const em=bg2.getObjectByName('embolo');
    if(em) em.position.y=0.07*(Math.sin(ang)-Math.sin(-0.20));
  }
  // aguja del manómetro
