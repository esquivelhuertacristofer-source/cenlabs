...
document.getElementById('panel').innerHTML =
  ...
  '<div class="secTitle">Autoevaluación</div>'+
...
function updateTele(){
  ...
  if(hide){
    html+='<div class="row"><span>τ = L/R</span><b>¿?</b></div>';
    html+='<div class="row"><span>t_off</span><b>¿?</b></div>';
    html+='<div class="row"><span>Vpico en switch</span><b>¿?</b></div>';
  } else if(dangerous){
...
function newReto(){
  ...
  // z15 (mayor Vz de las opciones) para que el presupuesto siga siendo válido si ese es el zener que el diseño realmente exige
  const vSwitchBudget=load.vsupply+ZENERS.z15.vz+d.vf+margin;
...
function clearDx(){
  const box=el('dxbtns'); if(!box)return;
  Array.prototype.forEach.call(box.querySelectorAll('button'),function(b){ b.classList.remove('right','wrong'); });
}
function refreshQuestion(){
  ...
  el('dxbtns').innerHTML=opciones.map(function(o,i){ return '<button class="b dx" data-i="'+i+'">'+o.t+'</button>'; }).join('');
