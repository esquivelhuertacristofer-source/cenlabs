...
function updateTele(){
  ...
  if(scenarioKey==='basetiempo'){
    ...
    if(!td.ok){
      el('t_t').textContent='— (ajusta base de tiempo)'; el('t_t').className='warn';
      el('t_f').textContent='—'; el('t_f').className='';
    } else if(!solved){
      el('t_t').textContent='✔ base correcta — cuenta las divisiones de un ciclo en pantalla'; el('t_t').className='good';
      el('t_f').textContent='—'; el('t_f').className='';
    } else {
      el('t_t').textContent=C1_T_MS.toFixed(2)+' ms'; el('t_t').className='good';
      el('t_f').textContent=C1_FREQ.toFixed(0)+' Hz'; el('t_f').className='good';
    }
    ...
    el('t_phase').textContent = solved
      ? 'Δt = '+dt.toFixed(3)+' ms → Δφ = '+ph.deg.toFixed(0)+'°'
      : 'Δt = '+dt.toFixed(3)+' ms (calcula Δφ con la fórmula del reto)';
