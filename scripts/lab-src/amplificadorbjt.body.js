...
(function () {
  ...
  function drawGraph(ctx) {
    ...
    // Punto Q (oculto en Predicción hasta verificar — su posición delata la respuesta)
    if (!state.hide) {
  ...
  function drawMedidorScreen() {
    ...
    if (state.hide) {
      c.fillStyle = '#8a8a8a'; c.font = 'bold 15px monospace';
      c.fillText('— predicción activa —', 14, 40);
      c.font = '12px monospace';
      c.fillText('valores ocultos hasta verificar', 14, 64);
  ...
  function drawScopeScreen() {
    ...
    c.stroke();
    if (state.hide) {
      c.fillStyle = '#8a8a8a'; c.font = '12px monospace';
      c.fillText('vout oculto — modo predicción', 8, midY + 4);
    } else {
      const ac = curSolveAC();
  ...
  function newReto() {
    ...
    // ICsat real (incluye RE, a diferencia de la fórmula anterior VCC/(2·RC) que la ignoraba
    // y podía pedir un ICQ objetivo por encima de ICsat — un reto imposible por construcción).
    const ICsatReal = (curVCC() - dev.vceSat) / (curRC() + curRE());
    const icTarget = ICsatReal * (0.3 + Math.random() * 0.35);
  ...
  function updateTele() {
    ...
    if (!t) return;
    if (state.hide) {
      t.innerHTML = '<div>IB ¿? · IC ¿? · VCE ¿?</div><div>gm ¿? · rπ ¿?</div><div>Av ¿? · swing± ¿?</div>';
      return;
    }
    const dc = curSolveDC();
    const ac = curSolveAC();
