...
const CASES={
  ratioSano:{
    tipo:'ratio', nombre:'Relación · unidad sana',
    ...
    mision:'Caso 1 · RELACIÓN EN VACÍO (unidad sana) — con X1-X2 en circuito abierto, aplica tensión reducida a H1-H2 y lee V2 para calcular la relación medida y compararla contra la tolerancia de placa.',
  },
  ratioFalla:{
    tipo:'ratio', nombre:'Relación · unidad con falla',
    unidad:'Transformador de control 500 VA, 220/24 V — unidad #2 (falla simulada)',
    aNameplate:220/24, kva:0.5, v1n:220, v2n:24,
    Vap:110, V2meas:11.50, scaleAp:150, scaleMeas:20,
    mision:'Caso 2 · RELACIÓN EN VACÍO (falla simulada) — misma prueba sobre una unidad distinta: compara si la relación medida sigue dentro de tolerancia.',
  },
  polAditiva:{
    tipo:'polaridad', nombre:'Polaridad · aditiva',
    unidad:'Transformador de control 150 VA, 240/24 V — clase ANSI aditiva',
  ...
  polSustractiva:{
    tipo:'polaridad', nombre:'Polaridad · sustractiva',
    unidad:'Transformador de distribución 300 kVA, 13200/240 V — clase ANSI sustractiva',
...
function ratioQuizText(key){
  ...
  return {pregunta, opciones:rotateArr(opciones,'ratio'+key)};
}
function polarityQuizText(key){
  ...
  return {pregunta, opciones:rotateArr(opciones,'pol'+key)};
}
...
document.getElementById('panel').innerHTML=`
  ...
  <div class="modebar" id="casebar">
    <button class="b on" id="c_ratioSano">Relación · sana</button>
    <button class="b" id="c_ratioFalla">Relación · con falla</button>
    <button class="b" id="c_polAditiva">Polaridad · aditiva</button>
    <button class="b" id="c_polSustractiva">Polaridad · sustractiva</button>
  </div>
  <div id="tele">
...
function updateTele(){
  ...
  if(c.tipo==='ratio'){
    ...
    set('t_est', active ? (st.pass?'CUMPLE tolerancia (±0.5%)':'NO CUMPLE tolerancia') : 'Banco apagado', active?(st.pass?'good':'bad'):'warn');
  }else{
    const st=polarityStats(caseKey);
    set('t_est', active ? ('Polaridad '+st.detected.toUpperCase()) : 'Banco apagado', active?'good':'warn');
  }
}
function updateReport(){
  ...
  if(!active){
    ...
    const st=ratioStats(caseKey);
    html=`<b>${c.mision}</b><br><span class="mono">Vap = ${c.Vap.toFixed(2)} V · V2 medida = ${c.V2meas.toFixed(2)} V<br>a medida = Vap/V2 = ${st.aMeasured.toFixed(4)} (nominal = ${c.aNameplate.toFixed(4)})<br>Desviación = ${st.devPct.toFixed(2)}% → ${st.pass?'CUMPLE':'NO CUMPLE'} ±0.5% (IEC 60076-1, Cl. 10)</span>`;
  }else{
    const st=polarityStats(caseKey); const aInv=1/c.aNameplate;
    html=`<b>${c.mision}</b><br><span class="mono">Vap = ${c.Vap.toFixed(2)} V (H1-H2, con H1↔X1 puenteado)<br>V medida (H2-X2) = ${st.Vmeas.toFixed(2)} V<br>N_BT/N_AT = 1/a = ${aInv.toFixed(5)}<br>V medida ${st.detected==='sustractiva'?'<':'>'} V aplicada ⇒ polaridad <b>${st.detected.toUpperCase()}</b></span>`;
  }
