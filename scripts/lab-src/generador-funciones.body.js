...
/* LA PANTALLA a la izquierda del frente y el mando a la derecha. Con 0,80 de
   alto centrada en 0,26 el borde superior caia en 0,66, por encima de la tapa
   de la caja (0,575): el marco quedaba en el aire. */
const screenMesh=new THREE.Mesh(new THREE.PlaneGeometry(0.80,0.64),new THREE.MeshBasicMaterial({map:scrTex}));
screenMesh.position.set(-0.36,0.16,0.526);
...
jackCh1.position.set(-0.56,-0.40,0.516);
   ...
   cuatro cosas por las que un aparato de banco se reconoce como aparato. */
{
  const marco=new THREE.Mesh(P3.extruido(
    [[-0.44,-0.36],[0.44,-0.36],[0.44,0.36],[-0.44,0.36]],
    {huecos:[[[-0.40,-0.32],[0.40,-0.32],[0.40,0.32],[-0.40,0.32]]],
     espesor:0.035,bisel:0.006}),MAT.knobBase);
  marco.position.set(-0.36,0.16,0.526-0.030);scopeGroup.add(marco);
  const asa=new THREE.Mesh(new THREE.TubeGeometry(new THREE.CatmullRomCurve3([
    new THREE.Vector3(-0.88,0.26,0),new THREE.Vector3(-0.88,0.64,0),
    new THREE.Vector3(0,0.78,0),
    new THREE.Vector3(0.88,0.64,0),new THREE.Vector3(0.88,0.26,0)],false),
  ...
  [[-1,-1],[1,-1],[-1,1],[1,1]].forEach(([sx,sz])=>{
    ...
    pa.position.set(sx*0.72,-0.60,sz*0.40);scopeGroup.add(pa);
  });
  for(let i=0;i<7;i++){
    const r=new THREE.Mesh(new THREE.BoxGeometry(1.00,0.014,0.035),MAT.knobRub);
    r.position.set(0,0.577,-0.36+i*0.06);scopeGroup.add(r);
  }
}
const termCap=new THREE.Mesh(new THREE.CylinderGeometry(0.075,0.075,0.12,16),MAT.termCap);
termCap.rotation.x=Math.PI/2;
termCap.position.set(-0.56,-0.40,0.585);
...
makeKnob(scopeGroup,0.45,0.22,0.526,'Volts/división','Fija en esta práctica para que la lectura dependa de la carga y no de la escala vertical.');
...
const cableGenScope=cableTube([[2.22,0.21,0.66],[0.15,0.75,1.05],[-2.16,0.32,0.43]],0.026,MAT.wireGen);
