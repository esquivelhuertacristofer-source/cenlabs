...
const RES={};
(function(){
  const g=new THREE.Group();g.position.set(1.6,0.62,0.6);
  const body=sh(new THREE.Mesh(new THREE.CylinderGeometry(0.05,0.05,0.22,16),std({color:0x9a815c,roughness:0.9,metalness:0})));
  body.rotation.z=Math.PI/2;g.add(body);
  [-1,1].forEach(s=>{
    const l=new THREE.Mesh(new THREE.CylinderGeometry(0.009,0.009,0.085,8),MAT.lead);
    l.rotation.z=Math.PI/2;l.position.x=s*0.152;g.add(l);
    const pin=new THREE.Mesh(new THREE.CylinderGeometry(0.009,0.009,0.06,8),MAT.lead);
    pin.position.set(s*0.19,-0.03,0);g.add(pin);
  });
  const bands=[];
  [-0.066,-0.022,0.022,0.078].forEach(bx=>{
    const b=new THREE.Mesh(new THREE.CylinderGeometry(0.052,0.052,0.022,16),std({color:0x1a1a1a,roughness:0.6,metalness:0}));
    b.rotation.z=Math.PI/2;b.position.x=bx;g.add(b);bands.push(b);
  });
  g.userData={special:'resistor'};
  scene.add(g);RES.g=g;RES.bands=bands;
  addHoverLabel(g,'Toca para leer las bandas','#4FD1C5',new THREE.Vector3(0,0.14,0),0.7);
})();
