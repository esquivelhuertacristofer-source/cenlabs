...
/* Los nombres con los que trabaja la biblioteca de piezas (`P3`, que el molde
   ya importa). Se rellena justo después de MAT. */
let MATP=null;
const MAT={
...
MATP={aluminio:MAT.bloque, acero:MAT.acero, cobre:MAT.crom, cromo:MAT.crom,
  chapa:MAT.acero, hierro:MAT.bloque, negro:MAT.caja, goma:MAT.goma,
  blanco:MAT.crom, ceramica:MAT.crom, papel:MAT.crom};
...
function construyeBanco(){
  ...
  // ---- los inyectores, del riel a la culata, con su manguera de retorno
  {
    ...
    for(let i=0;i<e.cil;i++){
      ...
      xIny.push(x);
      /* EL INYECTOR, con lo que tiene un inyector: cuerpo de bobina —o de pila
         piezoeléctrica—, conector, las juntas tóricas que lo sellan contra la
         culata y la tobera. Un cono truncado con una caja encima no enseña
         ninguna de las tres, y aquí el inyector es la pieza del laboratorio. */
      const cuerpo=P3.inyector({...MATP,acero:MAT.crom},{d:0.104,largo:0.46});
      cuerpo.position.set(x,d.yTapa+0.22,d.zRiel);
      cuerpo.traverse(o=>{if(o.isMesh)o.castShadow=true;});
      banco.add(cuerpo);
      const bob=roundedBox(0.10,0.13,0.10,MAT.caja,0.20);
      bob.position.set(x,d.yTapa+0.40,d.zRiel); banco.add(bob);
  ...
  // ---- las bujías incandescentes, en el otro costado de la culata
  {
    ...
    for(let i=0;i<e.cil;i++){
      const x=d.xRiel-d.largoRiel/2+paso*(i+0.5);
      /* La BUJÍA INCANDESCENTE, con su hexágono y su rosca: se aprieta con
         llave y se rompe por la rosca, que es la avería que todo el mundo teme
         al cambiarlas. Un tubo liso no dice ni por dónde se coge. */
      const b=new THREE.Mesh(P3.revolucion([
        [0,-0.10],[0.026,-0.10],[0.026,0.02],[0.038,0.02],
        [0.038,0.06],[0.026,0.06],[0.026,0.10],[0,0.10]],{seg:16}),MAT.bujia);
      b.rotation.z=0.42;
      b.position.set(x,d.yTapa-0.13,-d.ancho*0.30); banco.add(b);
      const hx=new THREE.Mesh(P3.revolucion(
        [[0,0],[0.040,0],[0.040,0.036],[0,0.036]],{seg:6}),MAT.bujia);
      hx.rotation.z=0.42; hx.position.set(x-0.026,d.yTapa-0.07,-d.ancho*0.30);
      banco.add(hx);
  ...
  // ---- el circuito de baja: depósito, filtro y bomba de alta
  {
    ...
    /* El filtro de gasóleo, EN CORTE: lo que filtra es el PAPEL PLISADO, y en
       un diésel ese papel hace además de decantador de agua —por eso lleva
       purga abajo—. Un bote cerrado no enseña ninguna de las dos cosas. */
    const f=new THREE.Mesh(P3.revolucion(
      [[0.092,-0.15],[0.100,-0.15],[0.100,0.15],[0.092,0.15]],
      {seg:24,fase:Math.PI*0.29,arco:Math.PI*1.42}),
      corte(std({color:0x2f3a46,roughness:0.55,metalness:0.30})));
    ...
    const pap=P3.filtroPlisado({papel:std({color:0xd9d2bc,roughness:0.92,metalness:0.03})},
      {rExt:0.082,rInt:0.050,alto:0.24,pliegues:30});
    pap.position.set(d.xFiltro,d.yFiltro,d.zFiltro); banco.add(pap);
    const pur=new THREE.Mesh(P3.revolucion(
      [[0,0],[0.024,0],[0.024,0.05],[0,0.05]],{seg:6}),MAT.crom);
    pur.position.set(d.xFiltro,d.yFiltro-0.19,d.zFiltro); banco.add(pur);
