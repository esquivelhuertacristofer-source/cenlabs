...
barrel.rotation.z=Math.PI/2;barrel.scale.set(0.135,X1-X0,0.135);barrel.position.set((X0+X1)/2,YC,ZC);scene.add(barrel);
const tapaA=new THREE.Mesh(GEO_CIL,MAT.tapa);tapaA.rotation.z=Math.PI/2;tapaA.scale.set(0.155,0.05,0.155);tapaA.position.set(X0,YC,ZC);scene.add(tapaA);
const tapaB=new THREE.Mesh(GEO_CIL,MAT.tapa);tapaB.rotation.z=Math.PI/2;tapaB.scale.set(0.155,0.05,0.155);tapaB.position.set(X1,YC,ZC);scene.add(tapaB);
const rod=new THREE.Mesh(GEO_CIL,MAT.rod);rod.rotation.z=Math.PI/2;scene.add(rod);
...
// --- etiquetas y picking ---
barrel.userData={title:'Cilindro de la estación',info:'El mismo banco horizontal sirve a las cinco estaciones: lo que cambia son la carrera, los tiempos de ida y vuelta y lo que el proceso exige. La carga vertical de la mesa elevadora vive en los números (la deriva del actuador suelto), no en el dibujo.'};
addHoverLabel(barrel,'Cilindro',AZUL,[(X0+X1)/2,YC+0.42,ZC],0.62);
...
function updateHW(){
  ...
  const lr=0.12+LROD*p;
  rod.scale.set(0.062,lr,0.062);rod.position.set(X1+lr/2,YC,ZC);
  carga.position.set(X1+lr+0.15,YC,ZC);
