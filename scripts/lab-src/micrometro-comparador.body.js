...
const spine=roundedBox(0.16,1.65,0.30,MAT.steelDk,0.02); spine.position.set(0,0.85,0); micG.add(spine);
spine.userData={title:'Bastidor (arco en C)',info:'Cuerpo rígido del micrómetro; mantiene el yunque y el husillo alineados en el mismo eje de medición.'};
const grip=roundedBox(0.13,0.5,0.27,MAT.thumb,0.03); grip.position.set(0,0.30,0.02); micG.add(grip);
grip.userData={title:'Empuñadura',info:'Zona de sujeción con la mano; aísla parcialmente el bastidor de la dilatación térmica de los dedos.'};
const topArm=roundedBox(0.85,0.15,0.26,MAT.steel,0.02); topArm.position.set(0.42,1.53,0); micG.add(topArm);
topArm.userData={title:'Brazo superior',info:'Sostiene el yunque fijo.'};
const bottomArm=roundedBox(0.85,0.15,0.26,MAT.steel,0.02); bottomArm.position.set(0.42,0.30,0); micG.add(bottomArm);
bottomArm.userData={title:'Brazo inferior',info:'Sostiene el manguito y guía el husillo.'};
...
const sleeve=sh(new THREE.Mesh(new THREE.CylinderGeometry(0.108,0.108,0.50,20),MAT.steel));
sleeve.position.set(ARM_X,0.60,0); micG.add(sleeve);
sleeve.userData={title:'Manguito (escala fija, mm y 0.5 mm)',info:'Porta la escala fija en milímetros y la línea de 0.5 mm; no se mueve, es solidario al bastidor.'};

const thimble=sh(new THREE.Mesh(new THREE.CylinderGeometry(0.128,0.128,0.28,20),MAT.steelDk));
thimble.position.set(ARM_X,THIMBLE_Y,0); micG.add(thimble);
thimble.userData={title:'Tambor (escala móvil, centésimas)',info:'50 divisiones × 0.01 mm = 0.5 mm por vuelta completa. Gira solidario al husillo.'};
...
lockLever.position.set(0.03,0.95,0.16); micG.add(lockLever);
...
const lockLabel=labelSprite('Tornillo de fijación','#ffd166'); lockLabel.position.set(0.25,1.15,0.3);
...
armH.rotation.z=Math.PI/2; armH.position.set(0.275,1.05,0); compG.add(armH);
const armV=sh(new THREE.Mesh(new THREE.CylinderGeometry(0.03,0.03,0.22,12),MAT.steelDk));
armV.position.set(0.55,0.93,0); compG.add(armV);

const dialBezel=sh(new THREE.Mesh(new THREE.CylinderGeometry(0.30,0.30,0.09,32),MAT.lock));
dialBezel.rotation.x=Math.PI/2; dialBezel.position.set(0.55,0.80,0.05); compG.add(dialBezel);
...
dialFace.position.set(0.55,0.80,0.10); compG.add(dialFace);
...
plungerHousing.position.set(0.55,0.50,0.10); compG.add(plungerHousing);
...
const dialLabel=labelSprite('Carátula · lectura','#4FD1C5'); dialLabel.position.set(0.55,1.15,0.15);
dialLabel.visible=false; dialLabel.raycast=()=>{}; compG.add(dialLabel); HOVER_LABELS.set(dialFace,dialLabel);
...
const plungerLabel=labelSprite('Palpador (husillo)','#ffd166'); plungerLabel.position.set(0.55,0.65,0.25);
plungerLabel.visible=false; plungerLabel.raycast=()=>{}; compG.add(plungerLabel); HOVER_LABELS.set(plungerHousing,plungerLabel);
...
function makePlanitudPlate(){
  ...
  const pts=[[-0.36,-0.22],[0.36,-0.22],[-0.36,0.22],[0.36,0.22],[0,0]];
