...
function renderConexion(){
  ...
  else if(sdbLinked===false) statusLine=`<span class="dtc">✗ Conexión NO exitosa tras 3 intentos.</span><br><span class="mono">Se registran las características del vehículo (marca, submarca, año modelo) y se notifica al propietario (Anexo normativo I, num. 2.6). Esto no equivale a un rechazo ni a una aprobación.</span>`;
...
function renderVerdict(){
  ...
  const bannerColor = v.code==='APROBADO'?'var(--good)':v.code==='RECHAZADO'?'var(--bad)':'var(--hv)';
  const bannerTxt = {APROBADO:'✔ APROBADO', RECHAZADO:'✘ RECHAZADO', NO_EVALUABLE:'⚠ NO EVALUABLE'}[v.code];
...
function updateTele(){
  ...
  const v=computeVerdict();
  const verB=el('t_ver');
  verB.textContent=v.code.replace('_',' ');
  verB.className=v.code==='APROBADO'?'good':v.code==='RECHAZADO'?'bad':'warn';
}
...
function drawToolScreen(){
  ...
  if(sdbLinked===false){
    ...
    c.font='13px Outfit, sans-serif'; c.fillStyle='#8FB3AC';
    wrapText(c,'Se registran las características del vehículo y se notifica al propietario.',18,168,282,17);
    scrTex.needsUpdate=true; return;
  }
  ...
  const v=computeVerdict();
  const bannerTxt={APROBADO:'APROBADO',RECHAZADO:'RECHAZADO',NO_EVALUABLE:'NO EVALUABLE'}[v.code];
  const bannerColor={APROBADO:'#34d399',RECHAZADO:'#f4475e',NO_EVALUABLE:'#FFB703'}[v.code];
