...
/* LA BOBINA, con el HILO A LA VISTA sobre su nucleo de tambor. Una inductancia
   son VUELTAS alrededor de un nucleo, y un bote liso esconde justo eso: por que
   una bobina de mas inductancia es mas gorda y por que satura antes. */
function makeInd3D(x,z){
  const g=P3.inductorTambor(MATP,{d:0.22,alto:0.17,vueltas:10,hilo:0.016,
    patas:0.11,nucleo:0x4a3a1f});
  ...
  const top=new THREE.Mesh(new THREE.CircleGeometry(0.100,20),new THREE.MeshBasicMaterial({map:tx,toneMapped:false}));
  top.rotation.x=-Math.PI/2;top.position.y=0.172;g.add(top);
  g.userData.el='L';
  scene.add(g);
  return{g,cv,tx};
}
...
/* EL DIODO DE RUEDA LIBRE, con su BANDA DE CATODO. Aqui la banda no es un
   adorno: este diodo solo hace su trabajo montado en un sentido, y montado al
   reves cortocircuita la fuente. Antes la banda estaba puesta AL LADO del
   cuerpo, no alrededor: no se leia como banda. */
function makeDiode3D(x,z){
  const g=P3.diodoAxial(MATP,{largo:0.14,d:0.066,patas:0.11,paso:0.28,
