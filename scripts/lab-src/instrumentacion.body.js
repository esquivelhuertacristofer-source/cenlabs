...
(function () {
  ...
  el('panel').innerHTML = `
    <div class="modebar">
      ${MODES.map((m) => `<button id="btn-mode-${m}" class="b">${MODE_META[m].icon} ${MODE_META[m].label}</button>`).join('')}
    </div>
    <div id="missionText" style="font-size:12px;color:#8FB3AC;margin-bottom:8px"></div>

    <div class="btns" style="margin-top:8px">
      <button id="btnRef" class="b" style="flex:1 0 48%">📍 Punto de referencia</button>
      <button id="btnHighError" class="b" style="flex:1 0 48%">⚠ Error alto (G↑, ε↓)</button>
    </div>

    <div class="btns" style="margin-top:8px">
      ${BRIDGE_TYPES.map((b) => `<button id="btn-bridge-${b.key}" class="b">${b.short}</button>`).join('')}
    </div>
    ...
    <div style="margin-top:10px;display:grid;grid-template-columns:1fr auto auto auto;gap:6px 8px;align-items:center;font-size:12px;color:#EAF4F1">
      <div>Vex</div><button class="b" id="vexDec" style="padding:2px 8px">−</button><span id="vVEX">—</span><button class="b" id="vexInc" style="padding:2px 8px">+</button>
      <div>ε</div><button class="b" id="epsDec" style="padding:2px 8px">−</button><span id="vEPS">—</span><button class="b" id="epsInc" style="padding:2px 8px">+</button>
      <div>G</div><button class="b" id="gDec" style="padding:2px 8px">−</button><span id="vG">—</span><button class="b" id="gInc" style="padding:2px 8px">+</button>
    </div>
    ...
    <div id="predWrap" style="display:none;margin-top:10px">
      ...
      <div class="btns" style="margin-top:6px">
        <button class="b" id="predNew" style="flex:1 0 48%">Nueva predicción</button>
        <button class="b" id="predCheck" style="flex:1 0 48%">Comprobar</button>
      </div>
    </div>

    <div id="sweepWrap" style="display:none;margin-top:10px">
      <button class="b" id="sweepRun" style="width:100%">▶ Ejecutar barrido de ε</button>
    </div>
    ...
    <div id="quizBox" style="margin-top:14px;border-top:1px solid #1E3A34;padding-top:10px"></div>

    <button id="autoTour" class="b" style="width:100%;margin-top:10px">▶ Recorrido guiado</button>
  `;
  ...
  function syncCtrlBtns() {
    MODES.forEach((m) => { el('btn-mode-' + m).className = 'b' + (state.mode === m ? ' on' : ''); });
    BRIDGE_TYPES.forEach((b) => { el('btn-bridge-' + b.key).className = 'b' + (curBridge() === b.key ? ' on' : ''); });
  ...
  function updateTele() {
    ...
    if (state.hide) { t.innerHTML = '<p style="color:#8FB3AC">— valores ocultos hasta responder —</p>'; return; }
    const res = curSolve();
    t.innerHTML = `
      <div class="g"><div class="gl"><span>Vbridge</span><b>${fmtMv(res.Vbridge)}</b></div></div>
      <div class="g"><div class="gl"><span>VoutIdeal</span><b>${fmtVAuto(res.VoutIdeal)}</b></div></div>
      <div class="g"><div class="gl"><span>Vcm</span><b>${fmtVAuto(res.Vcm)}</b></div></div>
      <div class="g"><div class="gl"><span>CMRR</span><b>${fmtDb(res.cmrrMinDb)} mín / ${fmtDb(res.cmrrTypDb)} típ</b></div></div>
      <div class="g"><div class="gl"><span>error (peor caso)</span><b>${fmtVAuto(res.errOutMin)} (${fmtPct(res.errPctMin)})</b></div></div>
      <div class="g"><div class="gl"><span>R_G</span><b>${fmtRG(res.RG)}</b></div></div>
    `;
  }
  ...
  function updateReport() {
    ...
    r.innerHTML = '<table class="repTable"><tr><th>ε</th><th>Vbridge</th><th>VoutIdeal</th><th>error%</th></tr>' +
  ...
  function refreshQuestion() {
    ...
    if (quizIdx >= QUIZ.length) { box.innerHTML = '<p style="color:#8FB3AC">✅ Cuestionario completo.</p>'; return; }
    ...
    item.a.forEach((a, i) => { html += `<button class="b quizOpt" data-i="${i}" style="width:100%;text-align:left;margin-bottom:4px">${a}</button>`; });
    box.innerHTML = html;
