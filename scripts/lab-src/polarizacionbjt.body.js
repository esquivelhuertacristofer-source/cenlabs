...
function solveBJT(topo,dev,beta,VCC,RC,RE,RB,R1,R2){
  ...
  const IBact=(VBB-VBE_ON)/(RBB+(beta+1)*RE);
  if(IBact<=0){
    return {region:'corte',IB:0,IC:0,IE:0,VCE:VCC,VBB:VBB,RBB:RBB};
  }
  const ICact=beta*IBact;
  const IEact=ICact+IBact;
  const VCEact=VCC-ICact*RC-IEact*RE;
  const ICsat=(VCC-dev.vceSat)/(RC+RE);
  if(ICact>=ICsat){
    const IC=ICsat;
    const IB=(VBB-VBE_ON-IC*RE)/(RBB+RE);
    const IE=IC+IB,VCE=dev.vceSat;
    return {region:'saturacion',IB:IB,IC:IC,IE:IE,VCE:VCE,VBB:VBB,RBB:RBB,betaForced:IC/IB};
  }
  return {region:'activa',IB:IBact,IC:ICact,IE:IEact,VCE:VCEact,VBB:VBB,RBB:RBB};
}
...
function makeBJT3D(){
  const g=P3.to92(MATP,{d:0.12,alto:0.17,patas:0.13,paso:0.032,
    cuerpo:MAT.bjtBody.color.getHex()});
...
function updateTele(){
  ...
  const hide=(mode==='predice'&&!predRevealed);
  if(hide){
    el('teleIB').textContent='¿?';
    el('teleIC').textContent='¿?';
    el('teleIE').textContent='¿?';
    el('teleVCE').textContent='¿?';
    el('teleRegion').textContent='¿?';
    el('teleRegion').style.color='#8a8a8a';
    return;
  }
