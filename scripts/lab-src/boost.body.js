...
(function () {
  ...
  function drawGraph(ctx) {
    ...
    // Ejes (seguros: no dependen de D, K ni del modo)
    ...
    if (state.hide) {
      ...
      ctx.fillText('Curva y Vout ocultos — predice antes de revelar', GXA + 4, (IL_Y0 + IL_Y1) / 2);
      return;
    }
  ...
  el('btnDCM').addEventListener('click', () => {
    state.iR = 6; refreshAll();
    const res = curSolveBoost();
    if (res.mode === 'DCM') {
      showToast('R=250Ω: carga ligera, K=' + res.K.toFixed(3) + ' cae por debajo de Kcrit=' + res.Kcrit.toFixed(3) + ' con L/D actuales → DCM', 'warn');
    } else {
      showToast('R=250Ω: con L/D actuales, K=' + res.K.toFixed(3) + ' sigue ≥ Kcrit=' + res.Kcrit.toFixed(3) + ' — sigue en CCM. Baja L o sube D para forzar DCM aquí.', 'info');
    }
  });
