...
S.setAnimate((dt,tGlob)=>{
  ...
  for(const p of PART){
    ...
    for(let i=0;i<p.ms.length;i++){
      // Modulo, no una resta: si el navegador se salta fotogramas (pestaña en
      // segundo plano, GC) dt puede valer segundos, la fase se pasa de 2 y
      // getPointAt(u>1) revienta dentro de three, matando el bucle de dibujo.
      p.fase[i]=(p.fase[i]+dt*v)%1;
