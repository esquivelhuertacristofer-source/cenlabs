...
(function () {
  ...
  function makeResistor3D() {
    const g = P3.resistencia(MATP, { largo: 0.26, d: 0.09, patas: 0.10, paso: 0.38,
  ...
  function drawGraph(ctx) {
    ...
    if (state.hide) {
      ...
      ctx.fillText('Curva oculta — predice antes de revelar', GX0 + 40, (GY0 + GY1) / 2 + 30);
      return;
    }
  ...
  const Y_PCB = 0.635;
  const raG = makeResistor3D(); raG.position.set(-0.7, Y_PCB, 1.0); benchG.add(raG);
  const rbG = makeResistor3D(); rbG.position.set(-0.1, Y_PCB, 1.0); benchG.add(rbG);
  const capG = makeCapacitor3D(); capG.position.set(0.5, Y_PCB, 1.0); benchG.add(capG);
  const icG = makeDIP3D(); icG.position.set(1.15, Y_PCB, 1.0); benchG.add(icG);
  S.scene.add(benchG);
  ...
  function drawScopeScreen() {
    ...
    if (state.hide) {
      ...
      ctx.fillText('Vc / Vout ocultos — predice antes de revelar', 12, canvas.height / 2);
      tex.needsUpdate = true;
      return;
    }
    const res = curSolve();
  ...
  function retoFeasible(target, tol) {
    for (const RA of RA_VALS) {
      if (RA < DEV.raMinRec) continue;
      for (const RB of RB_VALS) {
        for (const C of C_VALS) {
          const r = solveAstable(curVCC(), RA, RB, C);
          if (Math.abs(r.f - target) / target <= tol && r.duty <= 0.55) return true;
        }
      }
    }
    return false;
  ...
  function newReto() {
    ...
    do {
      ...
      target = solveAstable(curVCC(), RA_VALS[raI], RB_VALS[rbI], C_VALS[cI]).f;
      tries++;
    } while (!retoFeasible(target, 0.15) && tries < 200);
  ...
  window.__labDebug = {
    ...
    setMode, genPredice, checkPredice, newReto, checkReto, retoFeasible,
    runSweep, refreshAll,
  };
})();
