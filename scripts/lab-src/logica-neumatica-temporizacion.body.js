...
const f1=(x,n)=>{
  ...
  return x.toLocaleString('es-ES',{minimumFractionDigits:n,maximumFractionDigits:n});
};
/* ===== FIN MOTOR ===== */
...
// cilindro
const barrel=new THREE.Mesh(GEO_CIL,MAT.barrel);barrel.rotation.z=Math.PI/2;
barrel.position.set(3.02,0.70,0.35);scene.add(barrel);
const rod=new THREE.Mesh(GEO_CIL,MAT.rod);rod.rotation.z=Math.PI/2;
rod.position.set(3.58,0.70,0.35);scene.add(rod);
...
addHoverLabel(barrel,'Cilindro de la estación','#C7D6F5',[3.06,1.02,0.35],0.90);
...
barrel.userData={title:'Cilindro de la estación',info:'Diámetro y carrera vienen de la estación (ISO 15552). El banco conserva las proporciones del ciclo, no lo cronometra.'};
...
function updateHW(){
  ...
  barrel.scale.set(rB,0.84,rB);
  rod.scale.set(rR,0.62,rR);
  rod.position.x=3.58+0.30*animP;
  tip.position.x=3.90+0.30*animP;
  carga.position.x=4.06+0.30*animP;
  // LEDs
  CRIT.forEach((cr,i)=>{
    const okv=e[cr.k];
