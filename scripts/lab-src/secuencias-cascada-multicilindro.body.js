...
const S=createStage(mount,{cam:[5.4,3.0,7.6],target:[1.4,1.05,0.50],bgTop:'#0d1017',bgBot:'#05060a',bloom:0.42,minD:3.0,maxD:20});
const {scene}=S;
...
const EST=CILZ.map((z,i)=>{
  ...
  const barrel=new THREE.Mesh(GEO_CIL,MAT.barrel);barrel.rotation.z=Math.PI/2;g.add(barrel);
  const tapa1=new THREE.Mesh(GEO_CIL,MAT.tapa);tapa1.rotation.z=Math.PI/2;g.add(tapa1);
  const tapa2=new THREE.Mesh(GEO_CIL,MAT.tapa);tapa2.rotation.z=Math.PI/2;g.add(tapa2);
  const rod=new THREE.Mesh(GEO_CIL,MAT.rod);rod.rotation.z=Math.PI/2;g.add(rod);
  const carga=roundedBox(0.22,0.28,0.34,MAT.carga,0.03);g.add(carga);
  const fc0=new THREE.Mesh(new THREE.SphereGeometry(0.036,14,10),std(0x8f97a5,0.35,0.2));fc0.position.set(0.66,0.94,0);g.add(fc0);
  const fc1=new THREE.Mesh(new THREE.SphereGeometry(0.036,14,10),std(0x8f97a5,0.35,0.2));fc1.position.set(1.90,0.94,0);g.add(fc1);
  const val=roundedBox(0.36,0.24,0.34,MAT.valve,0.03);val.position.set(0.14,0.62,0);g.add(val);
  const spool=roundedBox(0.12,0.07,0.07,MAT.spool,0.02);spool.position.set(0.06,0.62,0.18);g.add(spool);
  const tA=tuboAB(MAT.tuboA), tB=tuboAB(MAT.tuboB);
  return {g,z,barrel,tapa1,tapa2,rod,carga,fc0,fc1,val,spool,tA,tB,soporte};
});
...
// etiquetas y picking
EST.forEach((st,i)=>{
  st.barrel.userData={title:'Cilindro '+String.fromCharCode(65+i),info:'Cilindro de doble efecto ISO 15552. El área anular es siempre menor que la del émbolo, así que a igual presión el retroceso da menos fuerza pero se hace más rápido.'};
  ...
  addHoverLabel(st.barrel,'Cilindro '+String.fromCharCode(65+i),AZUL,[1.25,1.18,st.z],0.62);
  addHoverLabel(st.val,'5/2 biestable',VIO,[0.14,0.92,st.z],0.58);
});
...
function updateHW(){
  ...
  EST.forEach((E,i)=>{
    ...
    E.barrel.scale.set(rB,1.10,rB);E.barrel.position.set(1.15,0.72,0);
    E.tapa1.scale.set(rB*1.15,0.05,rB*1.15);E.tapa1.position.set(0.62,0.72,0);
    E.tapa2.scale.set(rB*1.15,0.05,rB*1.15);E.tapa2.position.set(1.68,0.72,0);
    const lr=0.10+car*p;
    E.rod.scale.set(rR,lr,rR);E.rod.position.set(1.70+lr/2,0.72,0);
    E.carga.position.set(1.70+lr+0.13,0.72,0);
    E.fc1.position.set(1.70+0.10+car+0.10,0.94,0);
    ...
    E.spool.position.set(0.06+(p>0.5?0.16:0),0.62,0.18);
    setTuboAB(E.tA,[0.32,0.62,E.z+0.10],[0.62,0.72,E.z+0.10],0.018);
    setTuboAB(E.tB,[0.32,0.62,E.z-0.10],[1.68,0.72,E.z-0.10],0.018);
  });
  LIN.forEach((m,i)=>{
    m.visible=i<G;
    if(!m.visible)return;
...
const MODE_META={
  reparto:{nombre:'Reparto',cam:[[4.2,3.0,6.8],[1.20,1.05,0.50]],
