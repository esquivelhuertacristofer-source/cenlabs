...
/* LA PANTALLA a la izquierda y los mandos a la derecha, el reparto real del
   frente de un osciloscopio de banco. Ademas 0,88 de alto se salia por encima
   de la tapa de la caja, que esta en 0,65. */
const screenMesh=new THREE.Mesh(new THREE.PlaneGeometry(0.90,0.72),new THREE.MeshBasicMaterial({map:scrTex}));
screenMesh.position.set(-0.40,0.20,0.586);
...
probeCompTerm.position.set(-0.30,-0.44,0.585);
   ...
   cuatro cosas por las que un aparato de banco se reconoce como aparato. */
{
  ...
  marco.position.set(-0.40,0.20,0.586-0.030);scopeGroup.add(marco);
...
makeCosmeticKnob(0.36,0.38,'Volts/división','Fija en esta práctica para que la lectura dependa únicamente de la sonda y del ajuste de canal, no de la escala vertical.');
makeCosmeticKnob(0.36,0.04,'Base de tiempo','Fija en esta práctica; el foco está en la sonda, la compensación, los cursores y la FFT, no en el barrido horizontal.');
...
const cableProbe=cableTube([[-2.02,0.33,0.485],[-0.8,0.65,0.20],[0.523,0.60,-0.057]],0.026,MAT.wireProbe);
