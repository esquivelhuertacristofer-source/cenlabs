...
(function(){
  PIS.add(piston); PIS.add(aro1); PIS.add(aro2);
...
(function(){
  BIE.add(bCuerpo);
  bPie.rotation.x=Math.PI/2; bCab.rotation.x=Math.PI/2;
  BIE.add(bPie); BIE.add(bCab);
  marcaKey(BIE,'biela');
})();
...
const bujCer=cilY(MAT.aluminio), bujHex=cilY(MAT.acero), bujEle=cilY(emis('#c9a227',0.10));
...
(function(){
  BUJ.add(bujCer); BUJ.add(bujHex); BUJ.add(bujEle); BUJ.add(chispa);
  marcaKey(BUJ,'bujia');
})();
...
function aplicaGeom(){
  ...
  piston.scale.set(Rb-0.015,hp,Rb-0.015);
  aro1.scale.set(Rb+0.005,0.045,Rb+0.005);
  aro2.scale.set(Rb+0.005,0.045,Rb+0.005);
  bulon.scale.set(0.055,2*Rb*0.92,0.055);
  bPie.scale.set(0.085,0.30,0.085);
  bCab.scale.set(0.145,0.42,0.145);
  bCuerpo.scale.set(0.16,Lb,0.10);
  ...
  bujHex.scale.set(0.085,0.16,0.085); bujHex.position.set(0,GE.deck+0.10,0);
  bujCer.scale.set(0.065,0.34,0.065); bujCer.position.set(0,GE.deck+0.35,0);
  bujEle.scale.set(0.020,Math.max(0.05,hc*0.55),0.020);
  bujEle.position.set(0,GE.deck-hc*0.28,0);
  ...
  [[VAD,-1],[VES,1]].forEach(([V,s])=>{
    V.cab.scale.set(rV,1,rV); V.as.scale.set(rV*0.97,rV*0.97,1);
...
function pinta3D(){
  ...
  aro1.position.set(0,GE.hp*0.32,0); aro2.position.set(0,GE.hp*0.14,0);

  const cpx=a*st, cpy=CY+a*ct;
  BIE.position.set(cpx/2,(cpy+pinY)/2,0);
  BIE.rotation.z=Math.atan2(a*st,s);
  bPie.position.set(0,Lb/2,0); bCab.position.set(0,-Lb/2,0);
