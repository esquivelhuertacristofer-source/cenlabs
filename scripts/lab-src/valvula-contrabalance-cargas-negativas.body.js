...
const MATP={aluminio:MAT.motor, acero:MAT.acero, cobre:MAT.crom, cromo:MAT.crom,
  chapa:MAT.acero2, hierro:MAT.acero2, negro:MAT.valv, goma:MAT.cable,
...
(function(){
  ...
  /* EL MOTOR, con carcasa de aletas, patas y caja de bornes. Las aletas no son
     rayas pegadas al tubo: son la superficie por la que un motor cerrado echa
     todo su calor, y por eso un motor tapado por el polvo se quema aunque no le
     pidas ni un vatio de más. */
  const mot=P3.carcasaMotor(MATP,{d:0.34,largo:0.44,nAletas:16});
  mot.position.set(-0.42,0.92,0);
  mot.traverse(o=>{ if(o.isMesh) o.castShadow=true; }); gGrupo.add(mot);
...
(function(){
  ...
  /* EL MUELLE es una HÉLICE, no un anillo. Aquí importa: el tarado de la
     contrabalance es la fuerza de este muelle, y verlo comprimirse al apretar
     el tornillo es lo que hace que «subir el tarado» deje de ser una palabra. */
  cbMuelle=P3.muelle({acero:MAT.muelle},{r:0.052,largo:0.16,vueltas:6,hilo:0.011});
  cbMuelle.position.set(0,0.24,0); gCB.add(cbMuelle);
...
(function(){
  ...
  /* LOS CUATRO TIRANTES. Con una carga negativa colgando, la cámara resistente
     se INTENSIFICA por encima de la presión de la bomba —de eso va la práctica—,
     y quien aguanta esa presión contra las tapas son ellos. */
  for(let i=0;i<4;i++){
    const a=(i+0.5)/4*Math.PI*2;
    const t=new THREE.Mesh(new THREE.CylinderGeometry(0.020,0.020,1.16,10),MAT.acero2);
    t.position.set(2.35+Math.cos(a)*0.212,1.35,Math.sin(a)*0.212); gCil.add(t);
    for(const y of [0.80,1.90]){
      const tu=new THREE.Mesh(new THREE.CylinderGeometry(0.033,0.033,0.05,6),MAT.acero2);
      tu.position.set(2.35+Math.cos(a)*0.212,y,Math.sin(a)*0.212); gCil.add(tu);
    }
  }
...
function updateHW(){
  ...
  // Un muelle se comprime a lo LARGO, no se encoge por igual en las tres
  // direcciones: el diámetro del hilo no cambia al apretar el tornillo.
  cbMuelle.scale.y=1-0.26*kt;
