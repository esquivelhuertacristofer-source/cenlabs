...
function boardClick(u,v){
  ...
  if(best.act==='rheo')showToast('Reóstato de campo Rf = '+fmtOhm(Rf)+' — fija la pendiente de la recta Vt = Rf·If. Resistencia crítica a esta velocidad: Rc('+fmtRPM(nRpm)+') = '+fmtOhm(Rc(nRpm))+'.');
...
function setMode(k){
  ...
  if(k==='reto'){
    ...
    el('retoHint').textContent='Condición dada: Rf = '+fmtOhm(MYST.Rf)+', n = '+fmtRPM(MYST.n)
      +' · constantes conocidas: Em = '+Em+' V, Ik = '+Ik+' A, n0 = '+n0+' rpm · tolerancia: ±5 % en Vt (o ≈0 si predices colapso)';
  }
...
function buildQuiz(){
  ...
  if(MYST){
    const rc=Rc(MYST.n);
    QUIZ.reto={pregunta:'Para esta condición (Rf = '+fmtOhm(MYST.Rf)+', n = '+fmtRPM(MYST.n)+'), Rc(n) = '+fmtOhm(rc)+'. ¿Cómo se usa ese número para decidir si autoexcita?',
      opciones:[
        {t:'Si Rf < Rc(n) autoexcita; si Rf ≥ Rc(n) colapsa — aquí Rf '+(MYST.Rf<rc?'< Rc(n): SÍ autoexcita':'≥ Rc(n): NO autoexcita'),ok:true,
...
async function runAuto(){
  ...
  try{
    ...
    showToast('4/4 · Recorrido completo: explora otras combinaciones de Rf y n, o pide otro caso misterioso (🔀).');
