...
export const createBiologiaSlice: StateCreator<SimuladorState, [], [], any> = (set, get) => ({
  ...
  advanceRibosoma: () => set((state) => {
    ...
    const geneticCode: Record<string, { name: string, color: string }> = {
      ...
      'GCU': { name: 'Ala', color: '#10b981' },
      'GCC': { name: 'Ala', color: '#10b981' },
      'GCA': { name: 'Ala', color: '#10b981' },
      'GCG': { name: 'Ala', color: '#10b981' },
      'CGU': { name: 'Arg', color: '#ef4444' },
      'CGC': { name: 'Arg', color: '#ef4444' },
      'CGA': { name: 'Arg', color: '#ef4444' },
      'CGG': { name: 'Arg', color: '#ef4444' },
      'UGG': { name: 'Trp', color: '#8b5cf6' },
      'UAA': { name: 'STOP', color: '#000000' },
      'UAG': { name: 'STOP', color: '#000000' },
      'UGA': { name: 'STOP', color: '#000000' },
      // Fallback para otros codones en esta semilla
      'CAG': { name: 'Gln', color: '#f59e0b' },
      'GGU': { name: 'Gly', color: '#6b7280' },
      'UAC': { name: 'Tyr', color: '#ec4899' },
      'AUU': { name: 'Ile', color: '#f43f5e' }
    };

    const aa = { id: crypto.randomUUID(), ...(geneticCode[codon] || { name: 'AA', color: '#64748b' }) };
  ...
  validarB6: () => {
    const { evolucion } = get();
    const isOk = evolucion.generacion > 1;
