...
import {
  ...
  ChevronDown, ArrowRight, X, GraduationCap, Zap, Diamond,
} from "lucide-react";
...
export interface BriefingConfig {
  ...
  /** Retos o desafíos adicionales */
  retos?: string[];
  /** Color de acento primario (hex) */
  acento?: string;
  /** URL opcional de video (YouTube o MP4 directo) */
  videoUrl?: string;
}
        ...
        >
          ...
          >
            ...
            >
              ...
              {/* ── VIDEO INTRODUCTORIO (CINEMATIC PLAYER) ── */}
              {config.videoUrl && (
                <div className="relative group">
                  <div 
                    className="absolute -inset-1 rounded-[2rem] opacity-20 group-hover:opacity-40 transition-opacity blur-xl"
                    style={{ background: acento }}
                  />
                  <div className="relative rounded-[1.8rem] overflow-hidden border border-white/10 bg-black/40 aspect-video shadow-2xl">
                    <VideoPlayer url={config.videoUrl} acento={acento} />
                  </div>
                </div>
              )}
              ...
              {/* ── RETOS Y DESAFÍOS (NIVEL DIAMANTE) ── */}
              {config.retos && config.retos.length > 0 && (
                <Section title="Retos de Nivel Diamante" icon={<Zap size={14} />} acento={acento}>
                  <div className="grid grid-cols-1 gap-3">
                    {config.retos.map((reto, i) => (
                      <motion.div
                        key={i}
                        initial={{ opacity: 0, x: 20 }}
                        animate={{ opacity: 1, x: 0 }}
                        transition={{ delay: 0.3 + i * 0.1 }}
                        className="flex items-center gap-4 p-4 rounded-2xl border border-yellow-500/20 bg-yellow-500/5 hover:bg-yellow-500/10 transition-colors group"
                      >
                        <div className="w-10 h-10 rounded-xl bg-yellow-500/20 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                          <Diamond size={18} className="text-yellow-500" />
                        </div>
                        <p className="text-[12px] font-bold text-yellow-200/80 leading-snug">{reto}</p>
                      </motion.div>
                    ))}
                  </div>
                </Section>
...
// ─── Sub-componente: Cinematic Video Player ──────────────────────────────────
function VideoPlayer({ url, acento }: { url: string; acento: string }) {
  ...
  return (
    <video
      ...
      controls
      preload="metadata"
      poster="/assets/art/video_placeholder.webp"
    >
