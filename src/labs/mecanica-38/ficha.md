...
desarrollo:            # 🔒 los pasos técnicos — revisión experta
  ...
  - "Compara el mismo Vex y ε en ¼, ½ y puente completo: confirma que el medio puente y el puente completo escalan en razón exactamente 1:2 (ambas fórmulas son lineales), y que el cuarto de puente se aproxima a la razón 1:2:4 con una desviación ≤0.2% dentro del rango de ε de este simulador (±2000 µε), debido a su término no lineal (1+GF·ε/2) en el denominador — mientras que el error absoluto de salida (en voltios) permanece igual entre las tres, por lo que el error relativo (%) es siempre menor en el puente completo."
...
simulador_modela:      # 🔒
  ...
  - "Fórmulas exactas del medio puente (2 galgas activas en oposición) y del puente completo (4 galgas activas), ambas exactamente lineales: Vout/Vex=GF·ε/2 y Vout/Vex=GF·ε respectivamente — verificadas contra la misma nota AN078; confirmado que la relación de sensibilidad entre medio puente y puente completo es exactamente 1:2, y que el cuarto de puente se aproxima a la razón 1:2:4 con una desviación ≤0.2% dentro del rango ε de este simulador (±2000 µε) por su término no lineal — no es una razón exacta, a diferencia de medio/completo."
  ...
  - "Insight derivado (no una cita externa, sino una consecuencia matemática directa de las fórmulas anteriores): el error absoluto de salida inducido por CMRR depende únicamente de G, Vex (vía Vcm) y la tabla de CMRR — NO del tipo de puente elegido; pero como VoutIdeal escala en razón ≈1:2:4 entre ¼/½/puente completo para la misma ε (exacta entre ½ y completo; el cuarto de puente se desvía ≤0.2% dentro del rango ε de este simulador por su término no lineal), el error relativo (%) es siempre menor en el puente completo. Verificado con dos puntos de referencia numéricos: (a) Vex=5 V, ε=1000 µε, ¼ puente, G=100 → error≈0.1%; (b) mismo Vex/¼ puente, ε=100 µε, G=1000 → error≈1.0% (demuestra que el CMRR mínimo se estanca entre G=100 y G=1000, por lo que subir la ganancia para compensar una señal débil empeora el error relativo)."
