...
export const createQuimicaSlice: StateCreator<SimuladorState, [], [], any> = (set, get) => ({
  ...
  validarEstructura: () => {
    ...
    // Nivel Licenciatura: Validación basada en estabilidad y carga
    const isStable = Math.abs(protones - neutrones) <= (protones > 20 ? 1.5 : 2); // Simplificación técnica
    const isNeutral = protones === electrones;
    const isTarget = protones === 6 && (protones + neutrones) === 14; 

    if (isTarget && isNeutral) {
      set((state) => ({ 
        particulas: { ...state.particulas, isStable: true, message: "¡Éxito! Has forjado un Carbono-14 perfectamente equilibrado." }
      }));
      return true;
    } else {
      let msg = "Estructura inestable.";
      if (!isNeutral) msg = "El átomo tiene carga neta. Ajusta los electrones.";
      if (protones === 5) msg = "Has construido Boro, pero el objetivo es Carbono-14.";
      
      set((state) => ({ 
        particulas: { ...state.particulas, isStable: false, message: msg }
      }));
      return false;
    }
  },
