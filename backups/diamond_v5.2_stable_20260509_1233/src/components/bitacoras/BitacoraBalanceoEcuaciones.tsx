...
export default function BitacoraBalanceoEcuaciones({ onValidate }: { onValidate?: () => void }) {
  ...
  const completedCount = balanceo.reaccionesCompletadas?.length || 0;
  const canValidate = completedCount >= 4 && wordCount >= 50;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 font-['Outfit'] pb-20">
      ...
      <div className="bg-slate-50 rounded-[2rem] border border-slate-200 p-6 overflow-hidden relative shadow-sm">
         <div className="flex items-center gap-2 mb-4">
            <Microscope size={14} className="text-cyan-600" />
            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Monitor de Estabilidad Atómica</h4>
         </div>

         <div className="space-y-4">
            {/* Hallazgo: Conservación */}
            <div className={`p-4 rounded-2xl border transition-all flex items-center justify-between ${Math.abs(masaReactivos - masaProductos) < 0.01 ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-white border-slate-100'}`}>
               <div className="flex flex-col">
                  <span className={`text-[8px] font-black uppercase tracking-widest ${Math.abs(masaReactivos - masaProductos) < 0.01 ? 'text-emerald-600' : 'text-slate-400'}`}>
                     Masa Molar Total
                  </span>
               ...
               <div className={`text-xs font-black tabular-nums ${Math.abs(masaReactivos - masaProductos) < 0.01 ? 'text-emerald-600' : 'text-slate-400'}`}>
                 Δ {Math.abs(masaReactivos - masaProductos).toFixed(3)}
               </div>
            </div>

            {/* Hallazgo: Sincronía */}
            <div className={`p-4 rounded-2xl border transition-all flex items-center justify-between ${isBalanced ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-white border-slate-100'}`}>
               <div className="flex flex-col">
                  <span className={`text-[8px] font-black uppercase tracking-widest ${isBalanced ? 'text-emerald-600' : 'text-slate-400'}`}>
                     Estado de Fusión
                  </span>
                  <span className="text-xs font-bold text-[#023047]">
               ...
               <div className={`w-2 h-2 rounded-full ${isBalanced ? 'bg-emerald-500 shadow-[0_0_15px_#10b981]' : 'bg-slate-300 animate-pulse'}`} />
            </div>
         </div>
      </div>
      ...
      <div className="p-6 bg-white border border-slate-200 rounded-[2.5rem] shadow-sm space-y-4">
          ...
          <textarea 
             ...
             placeholder="Explica la importancia de la conservación de la materia en este proceso industrial..."
             rows={4}
      ...
      <div className="flex flex-col gap-3">
        <div className="flex gap-3">
          ...
          >
            <ShieldCheck size={16} />
            {canValidate ? 'Certificar Misión' : 'Incompleto'}
          </button>
        </div>
        <p className="text-[9px] font-bold text-slate-400 uppercase text-center tracking-widest px-4 leading-relaxed">
          {balanceo.reaccionesCompletadas?.length || 0} / 4 Reacciones balanceadas. Escribe al menos 50 palabras para habilitar la certificación.
        </p>
      </div>
