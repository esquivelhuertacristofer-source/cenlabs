    ...
    <div className="space-y-6 animate-in fade-in duration-500 font-['Outfit']">
      ...
      <div className="bg-slate-50 rounded-[2rem] border border-slate-200 p-6 overflow-hidden relative shadow-sm">
         ...
         <div className="space-y-4">
            ...
            <div className={`p-4 rounded-2xl border transition-all flex items-center justify-between ${Math.abs(V - 10) < 0.1 ? 'bg-green-500/10 border-green-500/30' : 'bg-white border-slate-100'}`}>
               <div className="flex flex-col">
                  <span className={`text-[8px] font-black uppercase tracking-widest ${Math.abs(V - 10) < 0.1 ? 'text-green-600' : 'text-slate-400'}`}>
                     Ley de Gay-Lussac (V)
                  </span>
               ...
               <div className={`w-2 h-2 rounded-full ${Math.abs(V - 10) < 0.1 ? 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]' : 'bg-red-500'}`} />
            </div>
            ...
            <div className={`p-4 rounded-2xl border transition-all flex items-center justify-between ${P < 5 ? 'bg-green-500/10 border-green-500/30' : P < 7 ? 'bg-yellow-500/10 border-yellow-500/30' : 'bg-red-500/10 border-red-500/30'}`}>
               <div className="flex flex-col">
                  <span className={`text-[8px] font-black uppercase tracking-widest ${P < 5 ? 'text-green-600' : P < 7 ? 'text-yellow-600' : 'text-red-600'}`}>
               ...
               <div className={`w-2 h-2 rounded-full ${P < 5 ? 'bg-green-500' : P < 7 ? 'bg-yellow-500 shadow-[0_0_10px_rgba(234,179,8,0.5)]' : 'bg-red-500'}`} />
            </div>

            {/* Hallazgo: Meta de Presión */}
            <div className={`p-4 rounded-2xl border transition-all flex items-center justify-between ${Math.abs(P - pTarget) < 0.1 ? 'bg-green-500/10 border-green-500/30' : 'bg-white border-slate-100'}`}>
               <div className="flex flex-col">
                  <span className={`text-[8px] font-black uppercase tracking-widest ${Math.abs(P - pTarget) < 0.1 ? 'text-green-600' : 'text-slate-400'}`}>
                     Objetivo de Telemetría
                  </span>
                  <span className="text-xs font-bold text-[#023047]">
                     {Math.abs(P - pTarget) < 0.1 ? '✓ Punto de Gay-Lussac OK' : `... Buscando ${pTarget.toFixed(2)} atm`}
                  </span>
               </div>
               <div className={`w-2 h-2 rounded-full ${Math.abs(P - pTarget) < 0.1 ? 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]' : 'bg-slate-300'}`} />
            </div>
         </div>
      </div>
