...
function DNASegment({ position, rotation, color, delay = 0, isActive }: { position: [number, number, number], rotation: [number, number, number], color: string, delay?: number, isActive: boolean }) {
...
function DNAHelix({ currentIndex }: { currentIndex: number }) {
  ...
  return (
    <group ref={groupRef}>
      {points.map((p, i) => {
        ...
        const pos1: [number, number, number] = [Math.cos(p.angle) * radius, p.y, Math.sin(p.angle) * radius];
        const pos2: [number, number, number] = [Math.cos(p.angle + Math.PI) * radius, p.y, Math.sin(p.angle + Math.PI) * radius];
        ...
        return (
          <group key={i}>
            ...
            <DNASegment 
              position={[(pos1[0] + pos2[0])/2, p.y, (pos1[2] + pos2[2])/2] as [number, number, number]} 
...
function Ribosome({ proteinaCount }: { proteinaCount: number }) {
  const groupRef = useRef<THREE.Group>(null!);
...
interface Sintesis3DSceneProps {
  fase: 'transcripcion' | 'traduccion';
  currentIndex: number;
  proteinaCount: number;
  adnPlantilla?: string;
}

export default function Sintesis3DScene({ fase, currentIndex, proteinaCount }: Sintesis3DSceneProps) {
  return (
