...
function Photon({ color, speed, intensity }: { color: string, speed: number, intensity: number }) {
...
function Thylakoid({ position, isActive, intensity }: { position: [number, number, number], isActive: boolean, intensity: number }) {
  const meshRef = useRef<THREE.Mesh>(null!);
...
function Granum({ position, isActive, intensity }: { position: [number, number, number], isActive: boolean, intensity: number }) {
  return (
...
function OxygenBubble({ delay }: { delay: number }) {
  const meshRef = useRef<THREE.Mesh>(null!);
  ...
  useFrame((state) => {
    ...
    if (meshRef.current) {
      ...
      (meshRef.current.material as THREE.MeshStandardMaterial).opacity = 1 - (t / 4);
    }
  });
...
function ChloroplastModel({ simulando, colorLuz, intensidad }: { simulando: boolean, colorLuz: string, intensidad: number }) {
...
export default function Fotosintesis3DScene({ simulando, colorLuz, intensidad }: { simulando: boolean, colorLuz: string, intensidad: number }) {
  ...
  const lightColor = lightColorMap[colorLuz] || '#ffffff';
