    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right duration-500">
        ...
        <div className="space-y-6">
           <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                 <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2">Padre 1</label>
                 <select 
                    value={padre1}
                    onChange={(e) => setGenetica({ padre1: e.target.value })}
                    className="w-full h-12 bg-slate-50 border border-slate-100 rounded-2xl px-4 text-xs font-black text-[#023047] focus:ring-2 focus:ring-indigo-500 outline-none"
                 >
                    {genotipos.map(g => <option key={g} value={g}>{g}</option>)}
                 </select>
              </div>
              <div className="space-y-2">
                 <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2">Padre 2</label>
                 <select 
                    value={padre2}
           ...
           <div className="space-y-3 pt-2">
              <div className="flex justify-between items-center">
                 <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Población (N)</span>
                 <span className="text-sm font-mono font-black text-[#023047]">{tamanioMuestra}</span>
              </div>
              <input 
                type="range" min="16" max="400" step="16"
           ...
           <button
             onClick={generarF1}
             className="w-full h-14 bg-[#4f46e5] text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-xl shadow-indigo-100 active:scale-95 transition-all flex items-center justify-center gap-2"
           >
      ...
      <div className="bg-[#1a1c2c] rounded-[2.5rem] p-6 text-white shadow-2xl relative overflow-hidden">
         ...
         <div className="flex items-center gap-4 mb-8">
            ...
            <div>
               ...
               <p className="text-[10px] text-indigo-400 font-bold uppercase tracking-widest">Conteo Empírico vs Teórico</p>
            </div>
         </div>
                  ...
                  <div key={f.id} className="bg-white/5 rounded-2xl p-4 border border-white/10">
                     <div className="flex justify-between items-center mb-2">
                        ...
                        <div className="flex gap-3">
                           <span className="text-[9px] font-bold text-white/40 italic">Teórico: {f.teorico.toFixed(1)}%</span>
                           <span className="text-[10px] font-black text-white">Real: {actual.toFixed(1)}%</span>
                        </div>
                     </div>
                     <div className="relative h-1.5 bg-white/10 rounded-full overflow-hidden">
                        ...
                        <motion.div 
                          ...
                          className="absolute h-full left-0 top-0 shadow-[0_0_100px_currentColor]"
                          style={{ width: `${actual}%`, backgroundColor: f.color }}
                        />
                     </div>
                  </div>
               );
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6">
         <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-xs font-black text-[#023047] uppercase text-[10px]">Análisis Estadístico</span>
         </div>
         <p className="text-[10px] font-bold text-slate-500 leading-relaxed italic">
            "Observa cómo al aumentar el tamaño de la muestra ($N$), los porcentajes reales se aproximan cada vez más a la predicción teórica de Mendel."
         </p>
      </div>
    </div>
  );
