  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center overflow-hidden font-sans">
      ...
      <div className="relative w-full max-w-[650px] aspect-square flex items-center justify-center z-10 scale-90 md:scale-100 lg:scale-110 transition-transform">
        ...
        >
          {/* Glow del núcleo */}
          <div className="absolute inset-0 bg-[#219EBC] blur-3xl opacity-20 rounded-full" />
          
          <div className="relative z-10 flex flex-wrap items-center justify-center gap-0.5 p-4">
             <AnimatePresence>
                {Array.from({ length: protones }).map((_, i) => (
                  <motion.div key={`p-${i}`} initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="flex-shrink-0">
                    <ParticleAsset 
                       type="proton" 
                       size={48} 
                       path={paths.proton} 
                ...
                {Array.from({ length: neutrones }).map((_, i) => (
                  <motion.div key={`n-${i}`} initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="flex-shrink-0">
                    <ParticleAsset 
                       type="neutron" 
                       size={48} 
                       path={paths.neutron} 
