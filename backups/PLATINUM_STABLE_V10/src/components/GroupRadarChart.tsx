...
export default function GroupRadarChart() {
  ...
  useEffect(() => {
    async function fetchGroupMetrics() {
      ...
      if (records) {
        // Mapeo rudimentario de materia basado en ID (mismo patrón que en planeamientos)
        ...
        const formatted = Object.entries(categories).map(([subject, stats]) => ({
          subject,
          value: stats.count > 0 ? Math.round(stats.total / stats.count * 10) : 0 // Normalizar a 100 si el score es 0-10
        }));
