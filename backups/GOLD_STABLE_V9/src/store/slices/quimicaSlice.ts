...
export const createQuimicaSlice: StateCreator<SimuladorState, [], [], any> = (set, get) => ({
  ...
  setParticulas: (p: number, n: number, e: number) => set((state) => ({ particulas: { ...state.particulas, protones: p, neutrones: n, electrones: e } })),
  validarEstructura: () => {
    const { protones, neutrones, electrones } = get().particulas;
    return protones === 6 && (protones + neutrones) === 14 && (protones - electrones) === 0;
  },
