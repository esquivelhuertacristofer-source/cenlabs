...
export function assessNuclearStability(Z: number, N: number): NuclearStatus {
  if (Z === 0 && N === 0) {
    return { level: 'critical', ratioNZ: 0, isKnownStable: false, decay: '—', label: 'Núcleo vacío', color: '#64748B' };
  }
  ...
  // Heurística para Z ≤ 10 (la banda de estabilidad vale ~N/Z ≈ 1.0)
  ...
  return { level: 'critical', ratioNZ, isKnownStable: false, decay: ratioNZ > 1.5 ? 'Desintegración α' : 'Emisión de protón', label: 'Altamente inestable', color: '#ef4444' };
}
